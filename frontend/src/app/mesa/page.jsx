'use client'; // Essencial para Next.js moderno (App Router)
...
export default function MesaPage() {
  ...
  useEffect(() => {
    // Rota relativa para a API que configuramos na Vercel
  ...
  return (
    <div style={{ padding: '60px 20px', backgroundColor: '#05070a', minHeight: '100vh', color: '#fff' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <header style={{ marginBottom: '40px', borderLeft: '3px solid #c5a47e', paddingLeft: '25px' }}>
          ...
          <h2 style={{ fontSize: '2.8rem', margin: '0', fontWeight: '500' }}>
            Mesa de Originação
          </h2>
          <div style={{ color: '#64748b', fontSize: '0.8rem', marginTop: '10px' }}>
            Exibindo **{filtered.length}** ativos estruturados.
          </div>
        </header>
            ...
            {filtered.slice(0, 40).map(asset => {
              ...
              return (
                <div key={asset.public_code} style={{ background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)', padding: '40px', borderRadius: '2px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                  ...
                  <h3 style={{ fontSize: '1.5rem', margin: '0 0 5px 0', color: '#fff' }}>{asset.city.toUpperCase()} / {asset.state}</h3>
                  <p style={{ color: '#94a3b8', fontSize: '0.75rem', marginBottom: '25px' }}>{asset.typology}</p>
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '20px', marginBottom: '30px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                      <span style={{ color: '#64748b', fontSize: '0.65rem', fontWeight: '700' }}>Avaliação de Mercado</span>
                      <span style={{ color: '#fff', fontWeight: '600' }}>{formatCurrency(vgv)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: '#c5a47e', fontSize: '0.65rem', fontWeight: '800' }}>Lance Mínimo Estimado</span>
                      <span style={{ color: '#c5a47e', fontWeight: '700' }}>{formatCurrency(lance)}</span>
                    </div>
                  </div>
                  <button onClick={() => handleAnalyze(asset)} style={{ marginTop: 'auto', width: '100%', padding: '14px', background: 'transparent', border: '1px solid #c5a47e', color: '#c5a47e', fontWeight: '700', cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.65rem' }}>
        ...
        {showModal && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: '#0a0d12', padding: '40px', border: '1px solid #c5a47e', maxWidth: '500px', width: '90%', textAlign: 'center' }}>
              <h3 style={{ fontSize: '1.8rem', marginBottom: '15px', color: '#fff' }}>Protocolo de Acesso</h3>
              <p style={{ color: '#94a3b8', fontSize: '0.85rem', marginBottom: '20px', textAlign: 'justify' }}>
                Para garantir a <strong>exclusividade da tese jurídica</strong>, o acesso ao Data Room requer a assinatura de um NDA. Este protocolo assegura que o ativo permaneça restrito a investidores qualificados.
              </p>
              ...
              <form onSubmit={goToZapSign}>
                <button type="submit" style={{ width: '100%', padding: '16px', background: '#c5a47e', color: '#05070a', border: 'none', fontWeight: '700', cursor: 'pointer', textTransform: 'uppercase' }}>
                  Assinar NDA e Acessar Dados
                </button>
                <button onClick={() => setShowModal(false)} style={{ width: '100%', marginTop: '15px', background: 'transparent', color: '#64748b', border: 'none', cursor: 'pointer', fontSize: '0.75rem' }}>
                  Voltar para a Mesa
                </button>
              </form>
            </div>
