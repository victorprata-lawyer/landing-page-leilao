...
const AssetShowcase = () => {
  ...
  const [typologyFilter] = useState(''); 
  const [hideLowValue, setHideLowValue] = useState(true); 
  const [loading, setLoading] = useState(true);

  // CONTROLE DO FLUXO JURÍDICO (ZAPSIGN)
  ...
  useEffect(() => {
    // PASSO 4: Ajustado para rota relativa. 
    // A Vercel entenderá que deve buscar na API do próprio domínio.
  ...
  // MOTOR DE CÁLCULO: MANTIDO (SEM BUG DE SÃO PAULO)
  const filtered = useMemo(() => {
    ...
    return assets.filter(asset => {
      ...
      const matchesTypology = !typologyFilter || asset.typology === typologyFilter;
      const matchesCity = !cityInput.trim() || normalize(asset.city).includes(normalize(cityInput));
      return matchesTypology && matchesCity;
    });
  }, [assets, cityInput, stateFilter, typologyFilter, hideLowValue]);
  ...
  const goToZapSign = (e) => {
    e.preventDefault();
    // Link direto do seu formulário NDA no ZapSign
  ...
  return (
    <div style={{ padding: '60px 20px', backgroundColor: '#05070a', minHeight: '100vh', fontFamily: 'Montserrat, sans-serif' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <header style={{ marginBottom: '40px', borderLeft: '3px solid #c5a47e', paddingLeft: '25px' }}>
          ...
          <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '2.8rem', margin: '0', color: '#fff', fontWeight: '500' }}>
            Mesa de Originação
          </h2>
          <div style={{ color: '#64748b', fontSize: '0.8rem', marginTop: '10px' }}>
            Exibindo **{filtered.length}** teses estruturadas.
          </div>
        </header>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '15px', marginBottom: '50px', padding: '20px', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '2px', alignItems: 'center' }}>
          <input
            type="text" placeholder="Cidade..." value={cityInput}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '0 15px', borderLeft: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <label htmlFor="qualityFilter" style={{ color: '#c5a47e', fontSize: '0.7rem', fontWeight: '700', cursor: 'pointer', textTransform: 'uppercase' }}>
              Filtro Qualificado (+300k)
            </label>
          </div>
        </div>

        {loading ? (
          <div style={{ color: '#c5a47e', textAlign: 'center', padding: '100px' }}>Sincronizando base de ativos...</div>
        ) : (
          <div key={`${stateFilter}-${cityInput}-${hideLowValue}`} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: '30px', alignItems: 'stretch' }}>
            {filtered.slice(0, 40).map(asset => {
              ...
              return (
                <div key={asset.public_code} style={{ background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)', padding: '40px', borderRadius: '2px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
                  ...
                  <div style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: '600', marginBottom: '8px', letterSpacing: '1px' }}>
                    {asset.public_code}
                  </div>
                  <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.5rem', margin: '0 0 5px 0', color: '#fff', fontWeight: '400' }}>
                    {asset.city.toUpperCase()} / {asset.state}
                  </h3>
                  ...
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '20px', marginBottom: '30px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                      ...
                      <span style={{ color: '#fff', fontWeight: '600', fontSize: '1rem' }}>{formatCurrency(vgv)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: '#c5a47e', fontSize: '0.65rem', fontWeight: '800', textTransform: 'uppercase' }}>Lance Mínimo Estimado</span>
                      <span style={{ color: '#c5a47e', fontWeight: '700', fontSize: '1rem' }}>{formatCurrency(lance)}</span>
                    </div>
                  </div>
                  <button onClick={() => handleAnalyze(asset)} style={{ marginTop: 'auto', width: '100%', padding: '14px', background: 'transparent', border: '1px solid #c5a47e', color: '#c5a47e', fontWeight: '700', cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.65rem', letterSpacing: '2px' }}>
        ...
        {/* MODAL COM EXPLICAÇÃO DO NDA E CAMPO DE CÓPIA */}
        {showModal && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: '#0a0d12', padding: '40px', border: '1px solid #c5a47e', maxWidth: '500px', width: '90%', textAlign: 'center' }}>
              <h3 style={{ fontFamily: 'Playfair Display', fontSize: '1.8rem', marginBottom: '15px', color: '#fff' }}>Protocolo de Acesso</h3>
              
              <p style={{ color: '#94a3b8', fontSize: '0.85rem', marginBottom: '20px', lineHeight: '1.6', textAlign: 'justify' }}>
                Para garantir a <strong>exclusividade da tese jurídica</strong> e a proteção das partes envolvidas, o acesso ao Data Room (matrículas e pareceres) requer a assinatura de um Acordo de Confidencialidade (NDA). Este protocolo assegura que o ativo permaneça restrito a investidores qualificados.
              </p>

              <p style={{ color: '#c5a47e', fontSize: '0.75rem', fontWeight: '700', marginBottom: '10px', textTransform: 'uppercase' }}>
                Código do Ativo para o Formulário:
              </p>
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '30px', background: 'rgba(255,255,255,0.05)', padding: '10px', border: '1px solid rgba(255,255,255,0.1)' }}>
                <div style={{ flex: 1, color: '#fff', fontWeight: '700', fontSize: '0.9rem', textAlign: 'left', alignSelf: 'center', paddingLeft: '10px' }}>
                  {selectedAsset?.public_code}
                </div>
                <button 
                  onClick={copyToClipboard}
                  style={{ background: copied ? '#27ae60' : '#c5a47e', color: '#05070a', border: 'none', padding: '8px 15px', fontWeight: '700', cursor: 'pointer', fontSize: '0.7rem', textTransform: 'uppercase', transition: '0.3s' }}
                >
